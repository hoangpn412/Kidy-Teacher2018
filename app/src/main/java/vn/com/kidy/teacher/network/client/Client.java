...
//    }
    ...
    public Observable<AlbumId> createAlbum(String schoolId, String classId, AlbumContent albumContent) {
        return getRetrofitService().createAlbum(schoolId, classId, albumContent).subscribeOn(Schedulers.io()).observeOn(AndroidSchedulers.mainThread());
    }

    @Override
    public Observable<ArrayList<Photo>> uploadImage(String token, MultipartBody.Part part) {
        return getRetrofitService().uploadImage(token, part).subscribeOn(Schedulers.io()).observeOn(AndroidSchedulers.mainThread());
    }
